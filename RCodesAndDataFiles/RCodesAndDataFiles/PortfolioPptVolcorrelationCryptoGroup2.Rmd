...
XRP1=XRPdata$Adj.Close
  
DOGEdata <- read.csv('DOGE_data_hourly.csv')

DOGE1=DOGEdata$Adj.Close

ADAdata <- read.csv('ADA_data_hourly.csv')

ADA1=ADAdata$Adj.Close

price_data1=cbind.data.frame(XRP1, DOGE1, ADA1)
names(price_data1)<-c("XRP","DOGE","ADA")
...
# Plot the data (Only for XRP. Change name to obtain plot for others)

plot(price_data1$XRP, type = "l", ylab='', main='Price (XRP)', cex.main = 2, cex.lab = 1.5, cex.axis = 1.5, lwd=4.0, col = "blue")
```
...
```{r}
p2 <- max_sr4 %>%
  gather(c('XRP', 'DOGE', 'ADA'), key = Asset,
         value = Weights) %>%
  mutate(Asset = as.factor(Asset)) %>%
  ggplot(aes(x = fct_reorder(Asset, Weights), y = Weights, fill = Asset)) +
  geom_bar(stat = 'identity') +
...
```{r}
row.names(Portfolios) <- XRPdata$X[-(1:11276)]
