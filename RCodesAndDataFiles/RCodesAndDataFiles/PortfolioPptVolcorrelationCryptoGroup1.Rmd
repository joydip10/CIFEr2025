...
BTC1=BTCdata$Adj.Close
  
ETHdata <- read.csv('ETH_data_hourly.csv')

ETH1=ETHdata$Adj.Close

BNBdata <- read.csv('BNB_data_hourly.csv')

BNB1=BNBdata$Adj.Close

price_data1=cbind.data.frame(BTC1, ETH1, BNB1)
names(price_data1)<-c("BTC","ETH","BNB")
...
# Plot the data (Only for BNB. Change name to obtain plot for others)

plot(price_data1$BTC, type = "l", ylab='', main='Price (BTC)', cex.main = 2, cex.lab = 1.5, cex.axis = 1.5, lwd=4.0, col = "red")
...
```{r}
stat<-matrix(0, nrow = 3, ncol = 7)
weight<-matrix(0, nrow = 3, ncol = 3)
for (i in 1:3){
...
```{r}
p1 <- cbind(min_var1[1:3], 8395*min_var1[4], sqrt(8395)*min_var1[5], sqrt(8395)*min_var1[10])
p2 <- cbind(min_var2[1:3], 8395*min_var2[4], sqrt(8395)*min_var2[6], sqrt(8395)*min_var2[11])
p3 <- cbind(min_var3[1:3], 8395*min_var3[4], sqrt(8395)*min_var3[7], sqrt(8395)*min_var3[12])
p4 <- cbind(min_var4[1:3], 8395*min_var4[4], sqrt(8395)*min_var4[8], sqrt(8395)*min_var4[13])
p5 <- cbind(min_mad[1:3], 8395*min_mad[4], sqrt(8395)*min_mad[9], sqrt(8395)*min_mad[14])
xtable(t(rbind(as.numeric(p1), as.numeric(p2), as.numeric(p3), as.numeric(p4), as.numeric(p5))), digits = 5)
```

```{r}
p1 <- cbind(max_sr1[1:3], 8395*max_sr1[4], sqrt(8395)*max_sr1[5], sqrt(8395)*max_sr1[10])
p2 <- cbind(max_sr2[1:3], 8395*max_sr2[4], sqrt(8395)*max_sr2[6], sqrt(8395)*max_sr2[11])
p3 <- cbind(max_sr3[1:3], 8395*max_sr3[4], sqrt(8395)*max_sr3[7], sqrt(8395)*max_sr3[12])
p4 <- cbind(max_sr4[1:3], 8395*max_sr4[4], sqrt(8395)*max_sr4[8], sqrt(8395)*max_sr4[13])
p5 <- cbind(max_sr5[1:3], 8395*max_sr5[4], sqrt(8395)*max_sr5[9], sqrt(8395)*max_sr5[14])
xtable(t(rbind(as.numeric(p1), as.numeric(p2), as.numeric(p3), as.numeric(p4), as.numeric(p5))), digits = 5)
```
...
```{r}
p2 <- max_sr4 %>%
  gather(c('BTC', 'ETH', 'BNB'), key = Asset,
         value = Weights) %>%
  mutate(Asset = as.factor(Asset)) %>%
  ggplot(aes(x = fct_reorder(Asset, Weights), y = Weights, fill = Asset)) +
  geom_bar(stat = 'identity') +
...
portfolio_values1_annual <- portfolio_values1 %>% mutate(Return = Return * 8395) %>% mutate(Risk1 = Risk1 * sqrt(8395), Risk2 = Risk2 * sqrt(8395), Risk3 = Risk3 * sqrt(8395), Risk4 = Risk4 * sqrt(8395), Risk5 = Risk5 * sqrt(8395)) %>% mutate(SharpeRatio1 = SharpeRatio1 * sqrt(8395), SharpeRatio2 = SharpeRatio2 * sqrt(8395), SharpeRatio3 = SharpeRatio3 * sqrt(8395), SharpeRatio4 = SharpeRatio4 * sqrt(8395), SharpeRatio5 = SharpeRatio5 * sqrt(8395))
```
...
```{r}
row.names(Portfolios) <- BTCdata$X[-(1:11276)]
